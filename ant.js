...
class Ant {
    ...
    draw(){
        noStroke();
        fill('red');
      rect(this.x * table.resolution, this.y * table.resolution, table.resolution, table.resolution);
    }
  
    update(){
        // Langton's_ant rules
      if (table.squareState(this.x, this.y)) { // the ant is in a black square
          this.turnleft();
      } else {  // the ant is in a white square
          this.turnright();
      }
      // in any case:
